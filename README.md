...
For shared services, the threat model can also be crossed with a separate impact
graph. The graph records which tenants, data stores, runtime domains, and recovery
domains a threat path can reach. The derived blast-radius report raises priority
without changing the human-authored threat model.

The graph can be bootstrapped from repository/IaC evidence, enriched from an AWS
read-only inventory, and checked with a graph-only attack-path simulation. None
of these steps deploys code, sends application traffic, or changes cloud state.
...
5. Derive blast radius from the reviewed threat graph and raise affected work.
6. Draft atomic requirements, lint identifiers and links, then render.
7. Re-run overlays against the written requirements to expose the assurance gap.
...
Kubernetes designs use the same pipeline rather than a separate checklist. The
Kubernetes input adapter normalizes YAML, rendered Helm/Kustomize output, or a
Terraform plan into the security graph. It derives tenant-boundary and
cluster-scope threat candidates, then emits `design_derived`,
`benchmark_check`, and `coverage_gap` work with verification plans. Runtime
comparison is read-only and treats drift as review work.
...
python3 -m pytest tests/                # deterministic layer, 807 tests
```
