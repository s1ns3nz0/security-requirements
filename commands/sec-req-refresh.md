...
Rebuild the blast-radius result from the reviewed graph. Keep the previous JSON
outside the overwrite path so the comparison can detect scope expansion:
...
An expanded path stops the refresh before requirements are merged. A reviewer
must decide whether the graph change is intended and update the requirements.

For Kubernetes, an optional read-only runtime snapshot can be compared with
the declared graph. The comparison does not collect Secret values and never
changes cluster state:

```
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/kubernetes_runtime_snapshot.py" \
    --design-graph .security-requirements/kubernetes-graph.yaml \
    --snapshot .security-requirements/kubernetes-runtime.json \
    --out .security-requirements/kubernetes-drift.json
```

`missing_runtime`, `unmanaged_runtime`, and `drift` results are review work;
they do not silently delete or weaken the declared requirement.
